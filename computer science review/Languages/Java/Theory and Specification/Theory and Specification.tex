...
\begin{itemize}
  \item overriding is a feature that allows a subclass or child class to provide a specific implementation of a method that is already provided by one of its super-classes or parent classes
  \item final methods can not be overridden
  \item you can call parent class method in overriding method using super keyword
\end{itemize}
...
\begin{itemize}
  ...
  \item the most common use of polymorphism in OOP occurs when a parent class reference is used to refer to a child class object
\end{itemize}

\question{Abstraction}

\begin{itemize}
  \item abstraction is a process of hiding the implementation details from the user, only the functionality will be provided to the user
  \item abstraction is achieved using abstract classes and interfaces
\end{itemize}

\question{Encapsulation}

\begin{itemize}
  \item encapsulation in Java is a mechanism of wrapping the data (variables) and code acting on the data (methods) together as a single unit
  \item the variables of a class will be hidden from other classes, and can be accessed only through the methods of their current class, also known as data hiding
  \item encapsulation in Java 
  \begin{itemize}
