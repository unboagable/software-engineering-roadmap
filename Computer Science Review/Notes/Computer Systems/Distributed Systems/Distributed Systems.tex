...
\begin{itemize}
  ...
  \begin{itemize}
    \item keep track of the essential file state within the process and be ready to recreate it
      \item migration system guarantees that higher  \item level file operations are atomic, operation opens the file, seeks as needed, performs a read or write, and then closes the file
      \item or maintain the same file state, but only reopen the file when needed
  \end{itemize}
  \item Migrating Communication Sessions
  \begin{itemize}
    \item need to reestablish and map gloabl state, such as network sockets
    \item higher level network name
    \item edge cases, such as what happens when a process move mid  \item transmission
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Class C (small): 24 bits(nework) + 8 bits (host) addess begin with 110
    \end{itemize}
    \item IP6
    \begin{itemize}
...
\begin{itemize}
    \item Dynamic Host Configuration Protocol (DHCP) allows IP addresses to be assiged on a temporary or quasi-permant basis
    \begin{itemize}
      \item DHCP is great for clients -- but not for servers: Servers need to have well-known addresses
    \end{itemize}
\end{itemize}
